...
public class Vino extends Bevanda implements Serializable
{	
	//Variabili di classe
	...
	public  Vino(IDEsterno t, Orologio dataCreazione)
	{
		super(t);
		this.dataCreazione = dataCreazione;
		this.gradoAlcolico = 0;
	}
	
	
	public  Vino(IDEsterno t, Orologio dataCreazione, double pA, double pV) // Usata Per Creazioni prodotto con Prezzo A/V
	{
		super(t);
		this.dataCreazione = dataCreazione;
		this.gradoAlcolico = 0;
	...
	public Vino() //Usato per creare un semplice oggetto che calcola indici
	{
		super();
	}
	/*
	public Vino(IDEsterno t, int gradoAlcolico)
	{
		super(t);
		this.gradoAlcolico = gradoAlcolico;
	}*/
	
	public int gradoAlcolico() 
	{
	...
	{
		if (!new File("Save/Vino.dat").exists()) 
		{
			//return false;
			throw new DeserializzazioneException("File Vino ancora non disponibile!");
		}
				
		DeserializzaOggetti load = new DeserializzaOggetti("Save/Vino.dat");
		try 
		{
	...
	{	
		...
		if (risp==JOptionPane.YES_OPTION) //Se SI allora risp = 0!
		{
			...
			{				
				...
				while(loop)
				{
					...
					{	
						System.out.println("MappaScadenza ora � vuota! Non ci sono pi� date di prodotti erogabili (buoni)!");
						loop=false;
					}
					else
					{
		...
		else //se la risposta � NO, aggiorna solo le richieste, e risp=1!
		{
	...
	public void ritiraScaduti(int quantita, String dataAmericana) throws GiacenzaInsufficienteException, ScadutiNonPresentiException, DataNonTrovataException
	{
		...
		//int quantoRitirare = ritiroScaduti;
			...
			while(loop)
			{
				...
				else  //Se non � stata inserita alcuna data
				{
					Set <Integer> ChiaviMappaScadenze2 = MappaDataScadenza.keySet();
					for (Integer n : ChiaviMappaScadenze2)
					{
	...
	{
		if (tRichieste > 0) return (double) nRichiesti/(double) tRichiesti;
        throw new ArithmeticException ("Al momento nessun/a " + this.getClass().getName() + " di questo tipo risulta ancora erogato/a. Impossibile calcolarne indice di gradimento!");
	}*/
	
	public double deperibilita() throws ArithmeticException
	{	//Cambio tScadute in nDepositate perchè voglio che mi dia 0% se non ci sono Scaduti
