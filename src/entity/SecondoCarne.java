...
public class SecondoCarne extends Secondo implements Serializable
{	
	...
	public SecondoCarne(IDEsterno t, Orologio dataCreazione)
	{
		super(t);
		this.dataCreazione = dataCreazione;
	}
	
	public  SecondoCarne(IDEsterno t, Orologio dataCreazione, double pA, double pV) // Usata Per Creazioni prodotto con Prezzo A/V
	{
		super(t);
		this.dataCreazione = dataCreazione;
	...
	public SecondoCarne() //Usato per creare un semplice oggetto che calcola indici
	{
		super();
	}
	...
	{
		if (!new File("Save/SecondoCarne.dat").exists()) 
		{
			//return false;
			throw new DeserializzazioneException("File SecondoCarne ancora non disponibile!");
		}
				
		DeserializzaOggetti load = new DeserializzaOggetti("Save/SecondoCarne.dat");
		try 
		{
	...
	public static void salvaVarStatiche() throws SerializzazioneException 
	{
		...
		String Dir = "Save";
		new File(Dir).mkdir();
		String path = Dir+"/SecondoCarne.dat";
	...
	//METODO (b) Calcolo Indici Gerarchia classi Dominio
			...
			public double gradimento(IDEsterno c) throws ArithmeticException
			{
				...
				else if(c.toString().equals(this.getClass().getName()))
				{
					...
					if (tSecondiErogati > 0) return (double) tErogate/(double) tSecondiErogati;
