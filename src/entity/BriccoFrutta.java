...
public class BriccoFrutta extends Bevanda implements Serializable
{	
	//Variabili di classe
	private static int tErogati=0;
	private static int tRichiesti=0;
	private static int tScaduti=0;
	private static int tDepositati=0;
	...
	public BriccoFrutta(IDEsterno t, Orologio dataCreazione)
	{
		super(t);
	...
	public  BriccoFrutta(IDEsterno t, Orologio dataCreazione, double pA, double pV) // Usata Per Creazioni prodotto con Prezzo A/V
	{
		super(t);
		this.dataCreazione = dataCreazione;
	...
	public BriccoFrutta() //Usato per creare un semplice oggetto che calcola indici
	{
		super();
	}
	...
	{
		if (!new File("Save/BriccoFrutta.dat").exists()) 
		{
			//return false;
			throw new DeserializzazioneException("File BriccoFrutta ancora non disponibile!");
		}
				
		DeserializzaOggetti load = new DeserializzaOggetti("Save/BriccoFrutta.dat");
		try 
		{
		...
		tDepositati = saveVarStatic.get("tDepositati");
		tBevandeDepositate += tDepositati;
		tPortateDepositate += tDepositati;    //Perchè ogni classe "base" deserializza
		
		tErogati = saveVarStatic.get("tErogati");
		tBevandeErogate += tErogati;
		tPortateErogate += tErogati;
		
		tRichiesti = saveVarStatic.get("tRichiesti");
		tBevandeRichieste += tRichiesti;
		tPortateRichieste += tRichiesti;
		
		tScaduti = saveVarStatic.get("tScaduti");
		tBevandeScadute += tScaduti;
		tPortateScadute += tScaduti;
	...
	public static void salvaVarStatiche() throws SerializzazioneException 
	{
		...
		if(saveVarStatic.containsKey("tDepositati"))  //Se già esiste lo rimuove e lo reinserisce
		{
			System.out.println("Chiave gi� esistente....aggiorno...");
			saveVarStatic.remove("tDepositati");
		}		
		saveVarStatic.put("tDepositati", new Integer(tDepositati));
		
		if(saveVarStatic.containsKey("tErogati"))  //Se già esiste lo rimuove e lo reinserisce
		{
			System.out.println("Chiave gi� esistente....aggiorno...");
			saveVarStatic.remove("tErogati");
		}		
		saveVarStatic.put("tErogati", new Integer(tErogati));
		
		if(saveVarStatic.containsKey("tRichiesti"))  //Se già esiste lo rimuove e lo reinserisce
		{
			System.out.println("Chiave gi� esistente....aggiorno...");
			saveVarStatic.remove("tRichiesti");
		}		
		saveVarStatic.put("tRichiesti", new Integer(tRichiesti));
		
		if(saveVarStatic.containsKey("tScaduti"))  //Se già esiste lo rimuove e lo reinserisce
		{
			//System.out.println("Chiave gi� esistente....aggiorno...");
			saveVarStatic.remove("tScaduti");
		}		
		saveVarStatic.put("tScaduti", new Integer(tScaduti));
		
		//Salvo la Mappa
		String Dir = "Save";
		new File(Dir).mkdir();
		String path = Dir+"/BriccoFrutta.dat";
	...
	public void deposita(int data, int quantita)
	{
		...
		tDepositati+=quantita;  //Prodotti Depositati di Tipo "Altro"
	...
	public void erogaLB(int quantita, Utente utenteCorrente) throws GiacenzaInsufficienteException, LineUnavailableException
	{		
		...
		if (i>=0)    //PRODOTTO BUONO
		{
			...
			log.logErogazione(utenteCorrente, this.iDEsterno.toString(), quantita, this.giacenza);
		}
		else  //PRODOTTO SCADUTO
		{
			...
			if(tScadutiSegnalati == nGiacenzaScaduti)  //Se tutti i prodotti scaduti sono stati tutti segnalati
			{
				...
				{
					erogaLB(quantita, utenteCorrente);
				}
				else
				{
	...
	{	
		...
		if (risp==JOptionPane.YES_OPTION) //Se SI allora risp = 0!
		{
			...
			tErogati += daErogare;    //E' il num max di prodotti erogabili! Questo è il caso in cui chiedo più di quanto c'è in giacenza!
	...
	public void ritiraScaduti(int quantita, String dataAmericana) throws GiacenzaInsufficienteException, ScadutiNonPresentiException, DataNonTrovataException
	{
		...
		//int quantoRitirare = ritiroScaduti;
			...
			tScaduti+=quantita;  // ***Errore corretto: "+="
	...
	{
		//System.out.println("Calcolo gradimento BriccoFrutta...");
		if (tErogati > 0) return (double) nErogati/(double) tErogati;
	...
	/*public double gradimentoMedioGiornaliero(Double gradimentoMedioGiornaliero) throws ArithmeticException
	{
		if (tErogate > 0) 
		{
	...
	{
		System.out.println("Calcolo deperibilità BriccoFrutta...");
	...
	public double successo(IDEsterno c) throws ArithmeticException
	{
		...
		{
			System.out.println("Calcolo successoClasse Bricco...");
			if (tRichiesti > 0) return (double) tErogati/(double) tRichiesti;
	...
	public double gradimento(IDEsterno c) throws ArithmeticException
	{
		...
		{
			System.out.println("Calcolo gradimentoClasse Bricco...");
			if (tBevandeErogate > 0) return (double) tErogati/(double) tBevandeErogate;
	        throw new ArithmeticException ("Al momento nessun/a " + this.getClass().getName() + " di questo tipo risulta ancora erogato/a. Impossibile calcolarne indice di gradimento!");
		}
		else 
	...
	public double deperibilita(IDEsterno c) throws ArithmeticException
	{
		...
		{
			System.out.println("Calcolo deperibilitàClasse Bricco...");
			if (tDepositati > 0) return (double) tScaduti/(double) tDepositati;
	...
	{
		String s = "\nClasse BriccoFrutta:\n" +
    			"tDepositate: "+tDepositati+"\n"+
    			"tErogate: "+tErogati+"\n"+
    			"tRichieste: "+tRichiesti+"\n"+
    			"tScadute: "+tScaduti+"\n";
		return s;
	}
